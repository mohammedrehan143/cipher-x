...
| Person | Role | Focus Area | Deliverables |
|---|---|---|---|
| **Person 1** | Satellite Data Engineer | Sentinel-2 Ingestion, Preprocessing & CVA | `change_magnitude.tif`, `change_mask.tif`, `spectral_delta.tif` |
| **Person 2** | GIS & Feature Engineer | Mask Vectorization, NDVI, Feature Extraction & Table Generation | `change_results.geojson`, `change_features.csv` |
| **Person 3** | ML Engineer | Label Creation, Random Forest Classifier & Inference | `predictions.csv`, `models/rf_classifier.joblib`, `models/rf_metadata.json` |
| **Person 4** | Frontend & GIS Visualizer | Streamlit Interactive GIS Dashboard & System Integration | `app/main.py`, `app/data_loader.py`, `PERSON4_PIPELINE.md` |
...
| Image Alignment | `src/preprocessing/align.py` | ✅ Done | `align_to_reference`, `align_images` (BUG-04 fixed) |
| Cloud Masking | `src/preprocessing/masking.py` | ✅ Done | `scl_to_mask`, `combine_masks`, `apply_mask` |
| CVA Computation | `src/cva/compute.py` | ✅ Done | `compute_delta`, `compute_magnitude`, `save_raster` (BUG-01 fixed) |
| Thresholding | `src/cva/threshold.py` | ✅ Done | `otsu_threshold`, `apply_threshold`, `clean_mask`, `save_change_mask` |
| Pipeline Runner | `run_pipeline.py` | ✅ Done | CLI orchestrator for Person 1 (BUG-06 fixed) |
...
| Module | File | Status | Description |
|---|---|---|---|
| Prototype Labeller | `src/models/labeller.py` | ✅ Done | Rule-based auto-labelling for prototype training set |
| Random Forest Classifier | `src/models/classifier.py` | ✅ Done | Data loading, median imputation, balanced RF training, evaluation |
| Classification Runner | `run_classify.py` | ✅ Done | End-to-end ML training & batch inference orchestrator |
...
## 5. Person 4 — Streamlit GIS Dashboard & Integration

### Owned Modules

| Module | File | Status | Description |
|---|---|---|---|
| Dashboard App | `app/main.py` | ⏳ Planned | Streamlit application with interactive GIS map, filters & KPIs |
| Data & Caching Engine | `app/data_loader.py` | ⏳ Planned | Artifact loader & robust fallback handling |
| GIS Map View | `app/map_view.py` | ⏳ Planned | Interactive Folium / Leaflet polygon visualization |
| Deep-Dive Inspector | `app/aoi_inspector.py` | ⏳ Planned | Per-AOI telemetry, spectral delta & ML explanation |
| Master Plan | `PERSON4_PIPELINE.md` | ✅ Complete | Complete 7-phase integration plan and audit |
